...
# - Store the best model in bestsvm                                       #
...
reg_larization = [1e4,5e4,1e5,5e5]

for lr in learning_rate:
    for reg in reg_larization:
        print('learning_rate ={}, regularization ={}'.format(lr,reg))
        classifier.train(xTrain,yTrain,lr,reg, iter =1500, verbose = True)
        param[(lr, reg)] =(classifier.calAccuracy(xTrain,yTrain), classifier.calAccuracy(xVal,yVal))
        BestSvm = Svm(xTrain.shape[1], numClasses)


        if classifier.calAccuracy(xVal,yVal)> bestAcc:
            bestAcc = classifier.calAccuracy(xVal, yVal)
            bestParameters [0] = lr
            bestParameters [1] = reg
            bestModel = BestSvm
...
for lr in learning_rate:
    for reg in reg_larization:
        ...
        classifier.train(xTrain,yTrain,lr,reg, iter =1500, verbose = True)
        param[(lr, reg)] =(classifier.calAccuracy(xTrain,yTrain), classifier.calAccuracy(xVal,yVal))
        BestSoftmax = Softmax(xTrain.shape[1], numClasses)
